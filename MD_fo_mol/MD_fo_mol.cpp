...
void denka::keisan(int n_th)
	...
	cout<<"initial potential energy  "<< (AMU*1e6/UNIT_E)*potential_energy() <<"  eV"<<endl;  //最後に [amu nm^2/ps^2] --> [eV]　へ変換
	cout<<"initial kinetic energy  "<< (AMU*1e6/UNIT_E)*kinetic_energy() <<"  eV"<<endl;  //最後に [amu nm^2/ps^2] --> [eV]　へ変換
	initialenergy = PE_plus_KE();  ////単位は [amu nm^2/ps^2]
	cout<<"initial total energy  "<< (AMU*1e6/UNIT_E)*initialenergy<<"  eV"<<endl;  //最後に [amu nm^2/ps^2] --> [eV]　へ変換
	...
	for(i=0; i<=n_step; i++)
		...
		if(t_charge_bu != 0){
			...
			if(t_CT == 0){
				...
				// for(k=0;k<PN;k++){	
				// charge_next[k] = charge[k] + (dt_temp - pow(dt_temp, 2)/2 + pow(dt_temp, 3) / 6 - pow(dt_temp, 4) / 24) * (charge_dest[k] - charge[k]); 
				// Taylor 4th, OK
				}
			else
				{
				for(k = 0; k < PN; k++)
					{
					// CT_flagがzeroのイオンに対して、電荷が最初からcharge_dest[k]
					if((CT_flag[k] * t_CT) == 0)
						{
